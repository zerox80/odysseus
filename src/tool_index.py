...
BUILTIN_TOOL_DESCRIPTIONS: Dict[str, str] = {
    "bash": "Run a bounded bash command in the isolated executor. It sees only the active workspace and has no app secrets, host filesystem, SSH material, Docker socket, or unrestricted network. Use it for workspace-local git, shell scripts, and Python; the executor image has no compilers or package installers, so it cannot build native code, install software, or perform host administration. Prefer a dedicated tool whenever one fits the job (file read/write/edit, search, listing). Do not use for web lookup/search; use web_search or web_fetch when web tools are available.",
    "python": "Execute bounded Python code in the isolated executor for workspace-local computation, data processing, math, scripting, and parsing. It has only the active workspace mount, not host files or app secrets. Not for writing code for the user. Prefer a dedicated tool for reading, writing, or searching files; use python only for what no dedicated tool covers. Do not use for web lookup/search; use web_search or web_fetch when web tools are available.",
    ...
    "download_model": "HIGH-TRUST: Download a HuggingFace model to a local or remote server. Requires ODYSSEUS_ENABLE_HIGH_TRUST_COOKBOOK=true; otherwise explain the opt-in and do not bypass it. Specify repo_id (e.g. 'Qwen/Qwen3-8B'), optional server host, and optional include filter for specific files.",
    "serve_model": "HIGH-TRUST: Start serving a model with vLLM, SGLang, llama.cpp, Ollama, or Diffusers. Requires ODYSSEUS_ENABLE_HIGH_TRUST_COOKBOOK=true; otherwise explain the opt-in and do not bypass it. cmd MUST start with the binary directly — e.g. `vllm serve /mnt/HADES/models/Qwen3.5-397B-A17B-AWQ --port 8003 --tensor-parallel-size 8 …`. NEVER prefix with `cd …`, `source …`, or chain with `&&`/`||` — those get rejected by the validator. The venv activation (env_prefix) and CUDA env are added automatically from the target host's saved settings. For image/inpainting/diffusion use python3 scripts/diffusion_server.py --model <repo> --port 8100. After launch, call list_served_models for readiness/errors and retry suggestions. If serve_model fails with 'Invalid characters in cmd', simplify to the bare binary + args.",
    "list_served_models": "HIGH-TRUST: List currently running model servers in the Cookbook. Requires ODYSSEUS_ENABLE_HIGH_TRUST_COOKBOOK=true because it inspects local processes and configured remote tasks. Shows status (loading, ready, idle, error), model name, port, throughput, and serve failure diagnosis/retry suggestions.",
    "stop_served_model": "HIGH-TRUST: Stop a running model server in the Cookbook by session ID or model name. Requires ODYSSEUS_ENABLE_HIGH_TRUST_COOKBOOK=true. Use when the user says 'kill my cookbook', 'stop the model', 'kill the serve', 'shut down vLLM', 'cancel the running model'.",
    "tail_serve_output": "HIGH-TRUST: Read the actual tmux stderr/traceback of a cookbook serve/download task. Requires ODYSSEUS_ENABLE_HIGH_TRUST_COOKBOOK=true. Use to debug WHY a task is `crashed`/`error` (compute_89 nvcc mismatch, OOM, missing kernels, wrong attention backend, etc.) so you can call serve_model with adjusted flags. Pass session_id from list_served_models; tail defaults to 300, bump if the error references 'see root cause above'.",
    "list_downloads": "HIGH-TRUST: List in-progress HuggingFace model downloads in the Cookbook. Requires ODYSSEUS_ENABLE_HIGH_TRUST_COOKBOOK=true because it inspects configured host tasks. Shows model name, phase, percent, session ID.",
    "cancel_download": "HIGH-TRUST: Cancel an in-progress model download by tmux session ID. Requires ODYSSEUS_ENABLE_HIGH_TRUST_COOKBOOK=true. Use for 'cancel the download', 'stop downloading X', 'kill the download'. Call list_downloads first to get the session_id.",
    ...
    "list_cached_models": "HIGH-TRUST: List models already cached on disk locally or on a remote host. Requires ODYSSEUS_ENABLE_HIGH_TRUST_COOKBOOK=true. Accepts friendly Cookbook server names like ajax. Use for 'what models do I have', 'show cached models', 'is X downloaded', 'list my models'. Avoids re-downloading.",
    ...
    "serve_preset": "HIGH-TRUST: Launch a saved Cookbook serve preset by name. Requires ODYSSEUS_ENABLE_HIGH_TRUST_COOKBOOK=true. Reuses the exact tmux command + host the user already saved. Use for 'run stable diffusion 3.5', 'serve vllm-qwen', 'start the inpaint model' — preset-name matches the user's UI labels.",
    "adopt_served_model": "HIGH-TRUST: Register an existing tmux model server (one started manually or outside the cookbook flow) into Cookbook tracking AND add it as a chat endpoint. Requires ODYSSEUS_ENABLE_HIGH_TRUST_COOKBOOK=true. Use when the user (or a previous turn) launched something via ssh+tmux and now wants it visible in the UI, stoppable via stop_served_model, and usable in the model picker.",
    ...
    "app_api": "Generic loopback to allowed Odysseus internal endpoints. Use this when the user wants something the UI can do but there's no named tool for it. Covers calendar, gallery, library/documents, memory, notes, tasks, settings, research, and compare. Sensitive auth/user/admin/shell paths and Cookbook host-control paths are blocked; do NOT use app_api to bypass ODYSSEUS_ENABLE_HIGH_TRUST_COOKBOOK for GPU inspection, process status, shell commands, package installs, engine rebuilds, or PID signalling. action='endpoints' with filter=<keyword> lists available endpoints. action='call' takes method+path+body. Hits same routes the UI uses. NOTE: themes are NOT an API endpoint — use the ui_control tool (create_theme / set_theme), not app_api. SESSIONS/CHATS: do NOT use app_api for these — GET /api/sessions returns EMPTY for tool calls (it's owner-filtered and tool calls authenticate as a different identity). EMAIL ACCOUNTS: do NOT use /api/email/accounts via app_api; use list_email_accounts, list_emails, and read_email instead. To list/rename/archive/delete/fork chats use the list_sessions and manage_session tools instead.",
    ...
    "manage_bg_jobs": "Inspect and control historical background-job records for this chat. New detached `#!bg` commands are disabled because generic tool execution is isolated and fail-closed. action='list' shows existing records (id/status/age/command); action='output' returns captured output; action='kill' stops a legacy running job by id. output/kill need a job_id from list.",
}

...
BUILTIN_TOOL_DESCRIPTIONS: Dict[str, str] = {
    "bash": "Run a bounded shell command in the isolated executor. It sees only the active workspace and has no app secrets, host filesystem, SSH material, Docker socket, or unrestricted network. Use it for workspace-local git, builds, and scripts; it cannot perform host administration. Prefer a dedicated tool whenever one fits the job (file read/write/edit, search, listing). Do not use for web lookup/search; use web_search or web_fetch when web tools are available.",
